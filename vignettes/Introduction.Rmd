...
data <- delim2xts(file_path = file_path, date_format = date_format, 
...
ACS=csCAS(param=c(acfs$ACF_params$Cauchy$beta,acfs$ACF_params$Cauchy$kappa),lag=1000) # CAS with b=3 and k=0.6
...
simARTA_Xts <- xts(x = simVector, order.by = dates)
...
```{r}
monthly_fits$GoF_monthly$January
```
...
for (i in 1:NumOfSeasons){
  PFXs[[i]] <- c(list(p0=eval(parse(text = paste0('monthly_sts$agg_list$',month.name[i],'$agg_stats[13,]'))),
                    Distr = match.fun(eval(parse(text = paste0('monthly_fits$params_monthly$',month.name[i],'$gamma3$Distribution$FXs'))))),
                   eval(parse(text = paste0('monthly_fits$params_monthly$',month.name[i],'$gamma3$Param'))))
}
...
simXts <- xts(x = simVector, order.by = dates)
