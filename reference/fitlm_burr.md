...
Fits the three-parameter Burr Type XII distribution to an xts series by
...
`Burr_InitValues` lookup table, and the scale is then derived
analytically from the sample first L-moment and the matched shapes'
unit-scale L-moment. The mean exists only when \\\theta \< 1\\, imposing
an effective upper bound on the tail index during optimisation. Zero
values below `zero_threshold` may be excluded via `ignore_zeros`.

## Usage

``` r
fitlm_burr(x, ignore_zeros = FALSE, zero_threshold = 0.01, order = c(1:5))
```
