#' @title nc2xts_nn
#'
#' @description Extracts nearest-neighbour point time series from one or more
#'   NetCDF files at the supplied coordinates, returning a plain \code{xts}
#'   object. For each requested point, the value of the grid cell whose centre
#'   is closest in Euclidean distance is read directly via \pkg{ncdf4}
#'   hyperslab indexing — no interpolation or extrapolation is performed.
#'   Points that fall outside the grid extent (with a half-cell buffer) yield
#'   an all-\code{NA} column in the output. Data are read one point at a time,
#'   each as a single-cell indexed hyperslab of all time steps, keeping the
#'   I/O footprint small. Multiple files (e.g. data downloaded in time chunks)
#'   are supported.
#'
#' @param filename Character vector; path(s) to one or more NetCDF files. If
#'   multiple files are given, results are row-bound and ordered by date.
#' @param varname Character; name of the variable to extract (a single
#'   variable name used across all files).
#' @param coords A matrix or data.frame of coordinates; the first two columns
#'   are taken as x (longitude) and y (latitude). Each row is a separate
#'   extraction point.
#'
#' @return An xts time series object with one column per supplied coordinate
#'   point. Columns are named after the variable for single-point extraction
#'   or \code{"Point1"}, \code{"Point2"}, etc. for multiple points.
#'
#' @examples
#' \dontrun{
